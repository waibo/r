...
class SaeRank extends SaeObject
{
    ...
    {
        $funname="create";
        if($this->emptyName($namespace, $number))
            return false;
    ...
    function set($namespace, $key, $value, $rankReturn=false)
    {
        if(!$rankReturn)
            $rankReturn=0;
        $funname="set";
        if($this->emptyName($namespace, $key, $value))
            return false;
    ...
    {
        $funname="getrank";
        if($this->emptyName($namespace, $key))
            return false;
        $mkey=substr($key,0,256);
    ...
    private function buildRet($ret)
    {
       ...
       {
           $this->_errno = -5;
           $this->_errmsg = "return value error";
    ...
    private function emptyName()
    {
        ...
        for($i=1;$i<$num;$i++)
        {
            ...
            {
                $this->_errno = -1;
                $this->_errmsg = "lack of parameters when call {$params[0]}";
                return true;
            }
        }
        return false;
